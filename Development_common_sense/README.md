...
OOP 로 코드를 작성하면 이미 작성한 코드에 대한 재사용성이 높다. 자주 사용되는 로직을 라이브러리로 만들어두면 계속해서 사용할 수 있으며 그 신뢰성을 확보 할 수 있다. 또한 라이브러리를 각종 예외상황에 맞게 잘 만들어두면 개발자가 사소한 실수를 하더라도 그 에러를 컴파일 단계에서 잡아낼 수 있으므로 버그 발생이 줄어든다 (캡슐화).  또한 내부적으로 어떻게 동작하는지 몰라도 개발자는 라이브러리가 제공하는 기능들을 사용할 수 있기 때문에 생산성이 높아지게 된다 (추상화). 객체 단위로 코드가 나눠져 작성되기 때문에 디버깅이 쉽고 유지보수에 용이하다. 또한 데이터 모델링을 할 때 객체와 매핑하는 것이 수월하기 때문에 요구사항을 보다 명확하게 파악하여 프로그래밍 할 수 있다.
...
### 한 줄 정의
객체 단위로 프로그램을 설계하고, 데이터와 기능을 함께 묶어 관리하는 프로그래밍 방식이다.

### OOP의 4대 핵심 개념
1) **캡슐화 (Encapsulation)**:
데이터와 기능을 하나로 묶고 외부에서 직접 접근하지 못하게 숨겨 상태를 보호하는 것

2) **추상화 (Abstraction)**:
복잡한 내부 구현은 숨기고 필요한 기능만 외부에 제공하는 것

4) **상속 (Inheritance)**:
기존 클래스를 재사용해 새로운 클래스를 만드는 것
공통 기능을 부모 클래스에 정의
...
### OOP의 장점
1) **코드 재사용성 증가**: 상속, 객체 조합으로 중복 코드 감소

2) **유지보수 용이**: 객체 단위로 수정 가능
     
3) **확장성**: 기존 코드 수정 없이 기능 추가 가능

4) **현실 세계 모델링에 유리**: 사람 중심 사고 방식과 유사

### OOP의 단점
1) **설계가 복잡해질 수 있음**: 객체 관계 설계가 어려움

2) **성능 오버헤드 가능**: 객체 간 메시지 호출 비용

3) **상태 관리 문제**: 객체가 상태를 가지므로 예측하기 어려운 버그 발생 가능
...
* Uniform Interface (일관된 인터페이스): API 구조가 일관되어야 함
...
* Entity 에 대한 내용은 body 에 담는다. (실제 데이터)
* 애플리케이션 서버가 행동할 판단의 근거가 되는 컨트롤 정보인 API 버전 정보, 응답받고자 하는 MIME 타입 등은 header 에 담는다. (요청을 어떻게 처리할지에 대한 정보)
* header 와 body 는 http header 와 http body 로 나눌 수도 있고, http body 에 들어가는 json 구조로 분리할 수도 있다. (meta -> 정보, data -> 실제 데이터)
...
4.  서버와 클라이언트가 같은 방식을 사용해서 요청하도록 한다. (클라이언트마다 요청 방식이 다르면 문제가 생김)
...
2.  멀티플랫폼 지원 및 연동이 용이하다. (REST는 단순히 HTTP 요청 + JSON만 있으면 된다. 그렇기에 하나의 API로 모든 플랫폼에 대응할 수 있다.)
3.  원하는 타입으로 데이터를 주고 받을 수 있다. (주로 JSON을 쓰지만, 다양한 형식을 보낼 수 있다.)
4.  기존 웹 인프라(HTTP)를 그대로 사용할 수 있다.

### 단점은 뭐가 있을까?

1.  사용할 수 있는 메소드가 한정적이다. (get, post, patch, put, delete)
2.  분산환경에는 부적합하다. (복잡한 서비스 간 통신에는 비효율적일 수 있다. 텍스트 기반 통신이라 데이터 크키가 크고, HTTP 요청/응답 구조 자체가 무겁기 때문이다.)
...
1) 순수 함수 (Pure function): 같은 입력 → 항상 같은 출력, 외부 상태를 변경하지 않음
2) 불변성 (Immutability): 데이터를 직접 수정하지 않음
3) 함수 조합 (Function composition): 작은 함수들을 조합해서 기능을 만듦
...
데이터가 들어올 때마다 자동으로 처리하여 데이터 값을 바꾸지 않고 즉시 계산해서 출력한다.

### 장단점
함수형 프로그래밍은 불변 데이터와 순수 함수를 기반으로 하기 때문에 코드의 예측 가능성이 높고 테스트와 동시성 처리에 유리하다는 장점이 있다. 반면 상태를 직접 변경하지 않기 때문에 성능 비용이 발생할 수 있고, 개념이 다소 추상적이어서 학습 난이도가 높다는 단점이 있다.
